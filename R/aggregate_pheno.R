...
#' @param pheno (optional) Output of `extract_pheno()` or `cut_seasons()`.
...
aggregate_pheno <- function(
  ...
  pheno_uids <- table(table(pheno[,paste(id, year, season)]))
  ...
  out_dt_l <- sapply(names(data), function(sel_id) {
    sapply(names(data[[sel_id]]), function(sel_year) {
      sapply(names(data[[sel_id]][[sel_year]]), function(sel_season) {
        window_metrics <- pheno[id == sel_id & year == sel_year & season == sel_season,]
        sel_data <- data[[sel_id]][[sel_year]][[sel_season]]$ts[
  ...
  if (all(!missing(pheno), include_pheno == TRUE)) {
    out_dt <- merge(pheno, out_dt, by = c("id", "year", "season"))
  }
