...
#' @param pop_seasons List with the allowed ranges for the dates of
#'  cycle peaks (one per season): each element is a character vector with two
#'  elements, in the form `"mm-dd"` (month-day), representing the range of the 
#'  temporal window (see default value as example) within the corresponding 
#'  season. The list must be of the same length of `seasons`.
#'  If `season` is one-length, `pop_seasons` can be both a 1-length list or 
#'  a vector.
#'  Each element of the list can be left to NA in order not to specify any
#'  filtering condition.
#' @param sos_seasons List with the allowed ranges for the dates of
...
assign_season <- function(
  ...
  if (!inherits(pop, "list") & !is.null(pop)) {pop <- list(pop)}
  ...
  if (!inherits(eos_win, "list") & !is.null(eos_win)) {sos_win <- list(eos_win)}
