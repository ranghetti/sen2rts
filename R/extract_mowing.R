...
#'
#'  **Experimental:** this function is still under active development;
#'  its arguments and outputs may change in future releases.
...
extract_mowing <- function(
  ...
  id <- value <- qa <- maxval <- relval <- relval_min <-
    begin <- end <- cycle <- start_mow <- end_mow <- mow_w <-
    x <- y <- NULL
